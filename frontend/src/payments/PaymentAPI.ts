...
export interface PropertiesResponse {
    status: string;
    data: {
        properties: Property[]; // Теперь явно указываем структуру
    };
    message?: string;
}
...
class PaymentApi {
     ...
     */
    static async getPropertiesWithUnpaidReceipts(): Promise<PropertiesResponse> {
        try {
            ...
            return {
                status: 'success',
                data: {
                    properties: response.data.properties || [] // Обращаемся к response.data.properties
                }
            };
        } catch (error: any) {
